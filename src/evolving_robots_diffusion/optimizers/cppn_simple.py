...
from evolving_robots_diffusion.simple_env.rewards import get_simple_reward
# Import classes and functions from cppn_helper (which contains Population and ParallelEvaluator)
from evolving_robots_diffusion.optimizers.cppn_neat.cppn_helpers import Population, ParallelEvaluator


from pathlib import Path

DEFAULT_NEAT_CONFIG_PATH = Path(__file__).resolve().parent / "cppn_neat" / "neat.cfg"
...
def eval_genome_fitness(genome, config, genome_id, generation):
    ...
    reward = get_simple_reward(robot)  # Example reward: sum of all material cells (simple heuristic)
    return reward
...
def run_cppn_simple(args, config_path=None):
    ...
    save_path = os.path.join("outputs", "simple_env", exp_name)
    try:
